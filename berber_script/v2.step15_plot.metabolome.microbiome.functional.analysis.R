...
pheno = c (lable1,  lable2)
phenobar = matrix (NA, ncol = length (pheno), nrow = length (kegg))
colnames (phenobar) = pheno
rownames (phenobar) = kegg

for (j in pheno) {
  tmp = p.adjust (cor_HbA1C$keggmodules [ , j, "p.value"], method="BH") ### adjust column wise on phenotypes
  for (k in kegg) {
    if (tmp [k] >= 0.1) {
      phenobar [k, j] = "grey"
    } else {
      if (cor_HbA1C$keggmodules [k , j, "estimate"] > 0) {
        phenobar [k, j] = "darkblue"
      } else if (cor_HbA1C$keggmodules [k , j, "estimate"] < 0){
...
colnames (phenobar) = c(lable1, lable2)
...
pdf (paste0(outdir, "/v2.heatmap_KEGG_vs_metabolite_clusters.pdf"), height = 10, width = 10)
heatmap.3 (plotmat,
           Rowv = dend, 
           Colv = dend2, 
           dendrogram = "column",
