...
###  here can add the other variable 

phenotypes2 <- phenotypes[rownames(metabolomic), ]
lable1 <- phenotype
lable2 <- paste0(phenotype, ".adj.partial")
...
tmpMat [, lable1, c ("estimate", "p.value")] =
  t (apply (metabolomic, MARGIN = 2, FUN = function (x) 
...
### Write to file
write.csv (output3, file = paste0(outdir, "/individual_metabolites.csv"), quote = F, row.names = T)
rm (output, output2, output3, tmpMat)
