...
final.fdr.cutoffs = 0.2 ### FDR thresholds, change to your likings.

vartotest_union <- list ()
vartotest_intersect <- list ()

#tmp2_name <- names(cor_HbA1C)[1,3]
for (tmp_name in names (cor_HbA1C)[c(1,3)]) {
  tmpMat = cor_HbA1C [[paste (tmp_name)]]
  vartotest_union [[paste ("fdr", final.fdr.cutoffs, sep = "_")]] [[tmp_name]] =
    unique (c (names (which (p.adjust (tmpMat [ , lable1, "p.value"], method = "BH") < final.fdr.cutoffs)), 
               names (which (p.adjust (tmpMat [ , lable2, "p.value"], method = "BH") < final.fdr.cutoffs))))
  vartotest_intersect [[paste("fdr", final.fdr.cutoffs, sep = "_")]] [[tmp_name]] =
    intersect (names (which (p.adjust (tmpMat[ ,  lable1, "p.value"], method = "BH") < final.fdr.cutoffs)),
               names (which (p.adjust (tmpMat[ ,  lable2, "p.value"], method = "BH") < final.fdr.cutoffs)))
}
