...
tmpMat [mgs.subset, lable1, c ("estimate", "p.value")] =
  t (apply (mgs_abundance [ctrl.no.na.4MGS, mgs.subset], MARGIN = 2, FUN = function (x) 
    unlist (cor.test (phenotypes [ctrl.no.na.4MGS, lable1], x,
                      method = cor_method, use = "pairwise.complete.obs") [c ("estimate", "p.value")])))       

### Associating MGSs with HOMA-IR while de-confounding for BMI    
tmpMat [mgs.subset, lable2, c ("estimate", "p.value")] =
  t (apply (mgs_abundance [ctrl.no.na.4MGS, mgs.subset], MARGIN = 2, FUN = function (x) 
    unlist (pcor.test (x = x, y = phenotypes [ctrl.no.na.4MGS, lable1], z = phenotypes [ctrl.no.na.4MGS, c("age", "sex", "BMI")],
                       method = cor_method) [c ("estimate", "p.value")])))       

cor_HbA1C [["MGSs"]] <- tmpMat
rm (tmpMat)
