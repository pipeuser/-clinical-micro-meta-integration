...
dimnames (tmpMat) [[2]] = c (lable1, lable2,"HbA1C_RichnessGenes7M.adj.partial")
...
KO_cor = apply (ko_abundance [ctrl.no.na.4KOs, ko.subset], MARGIN = 2, FUN = function (x) 
  cor (phenotypes [ctrl.no.na.4KOs,  lable1], x,
...
for (k in names (koann)) {
  ...
  if (length (incat) > 0 & length (notincat) > 0) {
    ...
    tmpMat [k, lable1, "p.value"] = x$p.value
    tmpMat [k, lable1, "estimate"] = (median (incat, na.rm = T) - median (notincat, na.rm = T))
  }
}
rm (KO_cor)
...
KO_cor = apply (ko_abundance [ctrl.no.na.4KOs, ko.subset], MARGIN = 2, FUN = function (x) 
  pcor.test (x = x, y = phenotypes [ctrl.no.na.4KOs, lable1], z = phenotypes [ctrl.no.na.4KOs, c("age", "sex", "BMI")],
...
for (k in names (koann)) {
  ...
  if (length (incat) > 0 & length (notincat) > 0) {
    x = wilcox.test (incat, notincat)
    tmpMat [k, lable2, "p.value"]  = x$p.value
    tmpMat [k, lable2, "estimate"] = (median (incat, na.rm = T) - median (notincat, na.rm = T))
  }
}
rm (KO_cor)
...
### First, obtain the correlation coefficients for partial Spearman correlation
### between KOs and HbA1C
# KO_cor = apply (ko_abundance [ctrl.no.na.4KOs, ko.subset], MARGIN = 2, FUN = function (x) 
#   pcor.test (x = x, y = phenotypes [ctrl.no.na.4KOs, "HbA1C"], z = phenotypes [ctrl.no.na.4KOs, "richnessGenes7M"],
#              method = cor_method) [1, "estimate"])      
...
#   if (length (incat) > 0 & length (notincat) > 0) {
#     x = wilcox.test (incat, notincat)
#     tmpMat [k, "HbA1C_RichnessGenes7M.adj.partial", "p.value"]  = x$p.value
#     tmpMat [k, "HbA1C_RichnessGenes7M.adj.partial", "estimate"] = (median (incat, na.rm = T) - median (notincat, na.rm = T))
#   }
# }
# rm (KO_cor)

cor_HbA1C [["keggmodules"]] <- tmpMat
